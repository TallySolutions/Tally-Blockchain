...
This sample provides an environment to display and capture metrics from the evm-network in real time. It consists of a docker-compose file that starts a Prometheus and Grafana server setup configured to collect and display metrics for the test network.
...
You will need to have installed **docker-compose with version 1.29 or above** (note that this is higher than the v1.14 requirement requested for the evm-network).

## How to use

1. Go to the evm-network directory and run bring up the evm-network **./network.sh up createChannel**
2. Bring up the Prometheus/Grafana network in the evm-network/prometheus-grafana directory and run **docker-compose up -d**
...
5. Deploy a chaincode (i.e. "./network.sh deployCC -ccn basic -ccp ../asset-transfer-basic/chaincode-go -ccl go"), start using the evm-network and use the Grafana dashboard to analyse and assess your network performances.
...
- `peer0.electorial.boatinthesea.com:9444`
- `peer0.voter.boatinthesea.com:9445`
- `orderer.boatinthesea.com:9443`
