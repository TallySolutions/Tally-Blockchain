...
FABRIC_CFG_PATH=${FABRICDIR}/config/
...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0electorial_${CC_NAME}_ccaas  \
                  --network ${NETWORK_NAME} \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest

    ${CONTAINER_CLI} run  --rm -d --name peer0voter_${CC_NAME}_ccaas \
                  --network ${NETWORK_NAME} \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0electorial_${CC_NAME}_ccaas  \
                  --network ${NETWORK_NAME} \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0voter_${CC_NAME}_ccaas  \
                  --network ${NETWORK_NAME} \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on peer0.electorial and peer0.voter
infoln "Installing chaincode on peer0.electorial..."
installChaincode electorial
infoln "Install chaincode on peer0.voter..."
installChaincode voter

## query whether the chaincode is installed
queryInstalled electorial

## approve the definition for electorial
approveForMyOrg electorial

## check whether the chaincode definition is ready to be committed
## expect electorial to have approved and voter not to
checkCommitReadiness electorial "\"ElectorialMSP\": true" "\"VoterMSP\": false"
checkCommitReadiness voter "\"ElectorialMSP\": true" "\"VoterMSP\": false"

## now approve also for voter
approveForMyOrg voter

## check whether the chaincode definition is ready to be committed
## expect them both to have approved
checkCommitReadiness electorial "\"ElectorialMSP\": true" "\"VoterMSP\": true"
checkCommitReadiness voter "\"ElectorialMSP\": true" "\"VoterMSP\": true"

## now that we know for sure both orgs have approved, commit the definition
commitChaincodeDefinition electorial voter

## query on both orgs to see that the definition committed successfully
queryCommitted electorial
queryCommitted voter
...
if [ "$CC_INIT_FCN" = "NA" ]; then
  ...
  chaincodeInvokeInit electorial voter
fi
