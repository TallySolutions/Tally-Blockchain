...
func main(){
	...
	timeoutGroup := router.Group("/", TimeoutMiddleware(timeout))
	{
		...
		timeoutGroup.PUT("/TallyScoreProject/createVoucher/:PAN", createVoucher)
	}
...
func createVoucher(c *gin.Context){
	...
	fmt.Printf("Initiating creation of voucher with ID= %s \n", VoucherID)
...
func createCompanyAsset(contract *client.Contract, businessPAN string){
	...
	fmt.Printf("\n Submit Transaction returned: O/p= %s , Error= %s \n", string(result), err)
	if err!=nil{
		fmt.Printf("Voucher was not created. Error.")
	}
	if err == nil{
