...
function startPeer()
{
   ...
   if [[ $PID -ne -1 ]]; then
      warnln "Peer server already running on peer node $1 [PID: $PID], skipping start."
      return 
   fi
   ...
   PID=$(checkIfRemoteProcessRunning peer $PEER_HOST_USER $PEER_HOST.$DOMAIN $PEER_HOST_KEY)
   if [[ $PID -ne -1 ]]; then
      successln "Peer Server started at peer node $1 [PID: $PID], you can check server log at $PEER_HOME_REMOTE/log.txt."
   else
