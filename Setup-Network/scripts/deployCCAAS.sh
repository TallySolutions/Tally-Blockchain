...
startDockerContainer() {
  ...
  if [ "$CCAAS_DOCKER_RUN" = "true" ]; then
    ...
    set -x
    ${CONTAINER_CLI} run --rm -d --name peer0tally_${CC_NAME}_ccaas  \
                  --network tally \
    ...
    infoln "    ${CONTAINER_CLI} run --rm -d --name peer0tally_${CC_NAME}_ccaas  \
                  --network tally \
                  -e CHAINCODE_SERVER_ADDRESS=0.0.0.0:${CCAAS_SERVER_PORT} \
                  -e CHAINCODE_ID=$PACKAGE_ID -e CORE_CHAINCODE_ID_NAME=$PACKAGE_ID \
                    ${CC_NAME}_ccaas_image:latest"
...
## Install chaincode on tbchlfdevpeer01.tally and tbchlfdevpeer01.org2
infoln "Installing chaincode on tbchlfdevpeer01.tally..."
installChaincode 1
infoln "Install chaincode on tbchlfdevpeer01.org2..."
installChaincode 2

## query whether the chaincode is installed
queryInstalled 1

## approve the definition for tally
approveForMyOrg 1

## check whether the chaincode definition is ready to be committed
## expect tally to have approved and org2 not to
checkCommitReadiness 1 "\"Tally\": true" "\"Org2MSP\": false"
checkCommitReadiness 2 "\"Tally\": true" "\"Org2MSP\": false"
...
checkCommitReadiness 1 "\"Tally\": true" "\"Org2MSP\": true"
checkCommitReadiness 2 "\"Tally\": true" "\"Org2MSP\": true"
